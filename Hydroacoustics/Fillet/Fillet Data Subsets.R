...
pwr.t.test(n = 267, #leaving n blank because that is the desired outcome
...
fillet_files <- read_csv("Fillet BlueView Filenames List.csv")
fillet_files_clean <- head(fillet_files, 6684)

## reformat the date 
fillet_files_clean$Date <- as.Date(fillet_files_clean$Date, format = "%m/%d/%y")

## create ON and OFF stratum if needed
fillet_ON <- subset(fillet_files_clean, fillet_files_clean$`Fillet status` == "ON")
fillet_OFF <- subset(fillet_files_clean, fillet_files_clean$`Fillet status` == "OFF")
